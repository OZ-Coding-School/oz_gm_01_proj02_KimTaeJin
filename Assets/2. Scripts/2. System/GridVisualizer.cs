...
public sealed class GridVisualizer : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        ...
        if ((origin - _lastOrigin).sqrMagnitude < 0.0001f) return;
        RefreshLines();
    }
    ...
    private void ApplyLineColor(LineRenderer lr)
    {
        if (lr == null || lr.material == null) return;
        var m = lr.material;
        if (m.HasProperty("_BaseColor")) m.SetColor("_BaseColor", lineColor);
        if (m.HasProperty("_Color")) m.SetColor("_Color", lineColor);
    }
